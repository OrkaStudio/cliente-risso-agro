-- ---------------------------------------------------------------------------
-- mover_animales v2: idempotencia + fecha real del hecho + contexto.
--
-- Un movimiento declarado sin señal tiene el mismo problema que un nacimiento,
-- con un agravante: reintentar HOY no duplica, pero **falla** — la segunda vez
-- no encuentra animales en el origen (ya se movieron) y levanta "No hay
-- animales para mover". O sea que la cola marca como error algo que en realidad
-- funcionó, y el productor ve un aviso rojo por una operación exitosa.
--
-- Con `p_alta_id` el reintento devuelve el resultado guardado del primer intento.
--
-- DROP + CREATE (no CREATE OR REPLACE): agregar parámetros genera una SOBRECARGA
-- y PostgREST queda con dos candidatas ambiguas, rompiendo el llamador viejo
-- (el diálogo Mover de Oficina). Los 9 parámetros originales conservan nombre,
-- tipo y orden. Ver leccion 2026-07-30-risso-agro-rpc-sobrecarga-postgrest.
-- ---------------------------------------------------------------------------
drop function if exists public.mover_animales(uuid, uuid, uuid, uuid, boolean, jsonb, uuid[], uuid, text);
...
begin
  ...
  -- `fecha` = la del hecho (la recorrida), no la de la sincronización, y el
  -- contexto del campo se suma al dato base sin pisarlo: un movimiento hecho
  -- desde Oficina sigue viéndose exactamente igual que antes.
